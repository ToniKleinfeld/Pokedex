...
async function singlePokemonData() {
  ...
  for (let index = countloadedPokemon; index < pokemonData.length; index++) {
    ...
    const promise = getPokemonData(pokeArrayIndex["url"]).then(
      (pokemonDataBase) => {
...
function getPokemonMovesAndAbilitys(pokeArrayIndex, pokemonDataBase) {
  ...
  for (let index = 1; index < pokemonDataBase.moves.length; index++) {
    pokeArrayIndex["moves"].push(
      pokemonDataBase["moves"][index]["move"]["name"]
    );
  }
  ...
  for (let index = 1; index < pokemonDataBase.abilities.length; index++) {
    pokeArrayIndex["abilities"][index] =
      pokemonDataBase["abilities"][index]["ability"]["name"];
  }
}
...
async function getSpeciesInfoData() {
  ...
  for (
    let index = countloadedPokemon - 25;
    index < pokemonData.length;
    index++
  ) {
    const pokeArrayIndex = pokemonData[index];

    const promise = getPokemonData(pokeArrayIndex["speciesurl"])
      .then((pokemonDataBase) => {
        if (pokemonDataBase.evolution_chain !== null) {
          pokeArrayIndex.evolution_chain =
            pokemonDataBase.evolution_chain.url.slice(26);
          pokeArrayIndex.flavor_text =
            pokemonDataBase.flavor_text_entries[2].flavor_text;
      ...
      .catch((error) => {
        console.error(
          `Fehler beim Laden der Species-Daten für ${pokeArrayIndex.name}:`,
          error
        );
      });
...
function renderPokecards() {
  for (
    ...
    document.getElementById("content").innerHTML += pokeCardHtml(
      pokemonInfo,
      index
    );
    if (pokemonInfo["typ2"] !== "") {
      document.getElementById(`typ${index}`).innerHTML +=
        pokeCardtyp2Html(pokemonInfo);
    }
  }
  loadingSpinner();
}
...
function loadBigPokecard(index) {
  ...
  document.getElementById("bigpokemoncard").innerHTML = bigPokemonCardhtml(
    pokemon,
    index
  );
...
function addAbilitys(pokemon, index) {
  document.getElementById(`abilitys${index}`).innerHTML += abilitysHtml(
    pokemon.abilities["0"]
  );
...
function pokeCardHtml(pokemon, index) {
  return /*html*/ `
     <div class="pokecard centercontainer column"  onclick="loadBigPokecard(${index})">
        <div class="pokeimg centercontainer"  style="background-color:${
          color[pokemon.typ1]
        }; background:linear-gradient(to right,${color[pokemon.typ1]},${
    color[pokemon.typ2]
  })">
        ...
        <div id="typ${index}" class="type centercontainer">
          <div class="typ centercontainer"  style="background-color:${
            color[pokemon.typ1]
          }"><strong>${pokemon["typ1"]}</strong></div>            
        </div>    
     </div>
    `;
}
...
function pokeCardtyp2Html(pokemon) {
  return /*html*/ `
    <div class="typ centercontainer" style="background-color:${
      color[pokemon.typ2]
    }"><strong>${pokemon["typ2"]}</strong></div>
  `;
}

function bigPokemonCardhtml(pokemon, index) {
  return /*html*/ `
                <div class="centercontainer big_pic " style="background-color:${
                  color[pokemon.typ1]
                }; background:linear-gradient(to right,${color[pokemon.typ1]},${
    color[pokemon.typ2]
  })"> 
            ...
            <div class="pokenamebigcard">
                <div class="centercontainer bigmenuname"><strong>${
                  pokemon.name
                }</strong></div>
                    ...
                    <tbody>
                        <table>
                            ...
                            <tr>
                                ...
                                <div class="borderstats">
                                <div class="progressbar "style="width:${
                                  pokemon.stats.s_defense
                                }%">${pokemon.stats.s_defense}</div>
                              </div>
                            </td>
                            </tr>
                            <tr>
